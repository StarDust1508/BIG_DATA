...
[![26 000+ строк](https://img.shields.io/badge/материала-26000%2B%20строк-blueviolet.svg)](#)
...
> **26 000+ строк** учебного материала. **7 модулей** + 3 сквозных проекта. **Полностью бесплатный.**

---

## 💡 О чём это в одну минуту

**Big Data** — это работа с данными, которые **не помещаются на одной машине** или приходят слишком быстро для обычной программы. Логи Яндекса за месяц, миллиарды банковских транзакций, поток событий с сайта в реальном времени — это **Big Data**.

**Apache Spark** — главный инструмент индустрии для такой работы. Это «Excel для распределённых данных»: он берёт ваши 500 ГБ и обрабатывает их параллельно на десятках машин — но вы пишете код, как будто работаете с одним большим DataFrame.

**Этот курс** — пошагово учит:
1. Понимать **архитектуру** (как устроено внутри: HDFS, Spark, Kafka).
2. Писать **PySpark-код** для реальных задач (ETL, joins, агрегации).
3. Обучать **ML-модели** на больших данных через Spark MLlib.
4. Соблюдать **закон** при работе с персональными данными (152-ФЗ, GDPR, AI Act) — это **уникальная** часть курса.

После прохождения у вас будет:
- ✅ Знания на уровне **Junior Data / ML Engineer**.
- ✅ **Готовое портфолио** на GitHub — 3 проекта с Model Card и юр.документацией.
- ✅ Понимание, **когда** Big Data действительно нужен, а когда хватит Pandas.
...
|------------|----------------|
| ML-программисту, хочет освоить Spark | Тому, кто ищет «нажми кнопку — стань Data Engineer» |
| Юристу, работающему с tech-командой | Тому, кому нужны видеолекции (здесь только текст и код) |
| Аналитику, желающему вырасти в DE | Тому, кому нужен платный сертификат с печатью |
| Начинающему программисту с Python-базой | Тому, кто хочет освоить Big Data за неделю |
| Тому, кто меняет профессию в IT (35+) | Тем, кто уже Senior DE и ищет advanced-только |
| Тимлиду — для команды | Тем, кто принципиально против «теории» |

**Базовое требование:** уверенный Python (циклы, функции, классы, словари). SQL — желательно.
...
## 🏆 Что выделяет этот курс на фоне других

- 🇷🇺 **На русском языке**, с гайдом «что писать на каком языке» в РФ-проектах.
- ⚖️ **Правовой контекст в каждом модуле** — 152-ФЗ, GDPR, AI Act, обезличивание, доказательственная сила ML. **Этого нет ни в одном бесплатном курсе индустрии.**
- 🧠 **Смысловые связи закрыты** — концептуальная карта, глоссарий на 80+ терминов, 583+ внутренних ссылок.
- 📈 **Mermaid-диаграммы рендерятся прямо на GitHub** — 13 живых схем архитектур (см. [ресурсы/диаграммы.md](./ресурсы/диаграммы.md)).
- 💼 **3 сквозных проекта** с **автогенерацией Model Card / Data Card** — готовое портфолио, не «hello world».
- 🎯 **Углублённые материалы** — Spark Internals, streaming, тестирование, MLOps, performance, интервью.
- 📊 **26 000+ строк** материала, **45 уроков**, **25+ практик**, **6 квизов + финальный экзамен на 50 вопросов**.
- 🔍 **Честный аудит** того, чего в курсе нет ([АУДИТ_СЛАБЫХ_МЕСТ.md](./АУДИТ_СЛАБЫХ_МЕСТ.md)) — нет маркетингового вранья.
...
**Что я смогу делать после прохождения?**
Уверенно: писать PySpark-код для ETL, строить ML-модели в MLlib, проектировать архитектуру pipeline'ов, читать чужой Spark-код, проходить интервью на Junior DE/ML позицию. Готовое **портфолио на GitHub** = 3 проекта, которые можно сразу показывать работодателю.

**Зачем юристу Big Data?**
Чтобы: **(а)** понимать, что технически возможно и что нет, когда консультируете компанию по 152-ФЗ; **(б)** разговаривать с ML-командами на одном языке; **(в)** грамотно работать с цифровыми доказательствами в суде. В РФ юрист с пониманием ML — **редкий специалист**, зарплата выше среднего по юр.рынку.

**Сколько часов в день нужно?**
В интенсивном режиме — 1.5–2 часа × 10–12 недель. В свободном — 5–6 месяцев. Меньше часа в день — забудется.

**Я ничего не знаю про программирование. Подойдёт?**
**Нет.** Сначала освойте базу Python (любой бесплатный курс на 1–2 месяца), потом возвращайтесь. Здесь предполагается, что вы пишете простые скрипты на Python.

**Нужен ли облачный кластер?**
Нет. Spark отлично работает локально на ноутбуке через `local[*]`. Облачные варианты (Databricks Community, Google Colab) — бесплатные альтернативы.

**Сколько денег нужно?**
**Ноль рублей.** Все технологии open source. Облачные варианты — бесплатные free tier. Курс на GitHub — без подписки.

**Можно ли пропустить Hadoop и сразу к Spark?**
Технически — да. Но настройки Spark (партиции, локальность данных, replication) опираются на концепции Hadoop. Один день на модуль 03 окупится.

**Я уже Middle DE. Что мне здесь?**
Углублённые материалы: Spark Internals, performance practice, streaming, MLOps, probabilistic structures. Плюс правовой блок — большинство Middle DE про него знают поверхностно.

**Курс заменит реальный опыт работы?**
**Нет.** Курс даёт **знания**. Уровень = знания + опыт + ответственность. Идите устраиваться на стажировку/Junior после курса.

**Где будут вопросы?**
Открывайте Issue в этом репозитории. Отвечу в течение дня-двух.

---

## 🚀 Поехали

1. Откройте [00_введение/УСТАНОВКА.md](./00_введение/УСТАНОВКА.md) — поставьте Python + Java + PySpark (30–60 минут).
2. Запустите `python 00_введение/hello_spark.py` — должно работать.
3. Откройте [01_основы_BigData/](./01_основы_BigData/) — первый урок про 5V.
4. Заведите [ХОД_ИЗУЧЕНИЯ.md](./ХОД_ИЗУЧЕНИЯ.md) и отмечайте прогресс.
5. **Делайте.** Курс — это не чтение. Это код. Запускайте, ломайте, чините.
...
Курс собран самостоятельно. Если нашли ошибку, у вас есть вопрос или хотите дополнить:
- 🐛 [Issues](https://github.com/StarDust1508/BIG_DATA/issues) — баги, опечатки, предложения.
- 🔀 [Pull Requests](https://github.com/StarDust1508/BIG_DATA/pulls) — доработки приветствуются (см. [CONTRIBUTING.md](./CONTRIBUTING.md)).
- ⭐ Поставьте звезду, если курс помог.

---

**Старт →** [00_введение/УСТАНОВКА.md](./00_введение/УСТАНОВКА.md)
